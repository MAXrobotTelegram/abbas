   ...
   Username = 'ندارد'
    ...
    if not value then
	 if result.id == tonumber(MKH) then
	   text = text..'مقام :بابای من - مدیر کل ربات (Executive Admin) \n\n'
         elseif result.id == tonumber(PRS) then
           text = text..'مقام: مدیر کل بات (Executive Admin) \n \n'
		...
		local value = redis:hget(hash, result.from.id)
		 if not value then
		    if result.from.peer_id == tonumber(MKH) then
		       text = text..'مقام :بابای من - مدیر کل ربات Executive Admin \n\n'
                    elseif result.from.peer_id == tonumber(PRS) then
                       text = text..'مقام: مدیر کل بات (Executive Admin) \n\n'
	...
	if hash then
	  ...
	  if not value then
		if msg.from.id == tonumber(MKH) then
		 text = text..'مقام : بابای من - مدیر کل ربات (Executive Admin) \n\n'
                elseif msg.from.id == tonumber(PRS) then
                 text = text..'مقام: مدیر کل بات (Executive ADmin) \n\n'

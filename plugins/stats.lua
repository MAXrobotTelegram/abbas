...
local function run(msg, matches)
  if matches[1]:lower() == 'nortteam' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /nortteam ")
    return about
  end 
  ...
  if matches[1]:lower() == "stats" then
    ...
    if matches[2] == "nortteam" then -- Put everything you like :)
